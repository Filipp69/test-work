...
let commonJsLibs = [
    ...
    // 'app/js/custom-libs/password-toggle-toggle.js',
...
function tempJS() {
    return gulp.src(['app/js/backend-temp.js', 'app/js/dev-temp.js'])
...
function bundleJS() {
    return gulp.src(jsLibs)
        ...
        gulp.src(['app/blocks/**/*.js', 'app/pages/**/*.js', 'app/js/main.js'])
            .pipe(concat('scripts.js'))
            .pipe(gulp.dest('dist/js'))
            .on('end', browserSync.reload),

        gulp.src(commonJsLibs)
            .pipe(concat('common.js'))
            .pipe(gulp.dest('dist/js'))
            .on('end', browserSync.reload);
}
function faviconSvg() {
    return gulp.src('app/favicon.svg')
        .pipe(gulp.dest('dist/'))
        .on('end', browserSync.reload);
}
function images() {
...
gulp.task('build', gulp.series(clean, icons, style, images, fontsTransfer,  faviconSvg, bundleJS, tempJS, html, function (done) {
        done();
    })
);
