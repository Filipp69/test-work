class Modal {
    constructor() {
        ...
        this.validFormModal = new formValidator('[data-validate-modal]', {});
    }
    clickShow() {
        this.showElems.forEach(button => {
            button.addEventListener('click', () => {
                ...
                if (parentModal) {
                    ...
                    let parentElem = document.querySelector(`[data-modal=${parentModalName}]`);
                    fadeOut({
                        el: parentElem,
                        timeout: 500
                    });
    ...
    create(modal, data) {
        ...
        JSON.parse(data).forEach(dataEl => {
            modalItemClone.querySelector('[data-modal-img]').setAttribute('src', dataEl.image);
            modalItemClone.querySelector('[data-modal-title]').textContent = dataEl.title;
            modalItemClone.querySelector('[data-modal-price]').textContent = dataEl.price;
