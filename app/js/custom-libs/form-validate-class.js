class formValidator {
  ...
  clear() {
      this.parentForm.forEach((formSingle) => {
          ...
          errorsFields.forEach((field) => {
              this._removeFieldInvalidClass(field);
          });
  ...
  isFieldValid(formSingle) {
      ...
      noValidate.forEach((item) => {
          this._removeFieldInvalidClass(item);
      });
  ...
  _validateEmail(field) {
      if (field) {
          ...
          if (value.search(pattern) !== 0 && value !== '') {
              this._addFieldInvalidClass(field);
          } else {
              this._removeFieldInvalidClass(field);
          }
      }
      ...
      if (field) {
          field.forEach((singleField) => {
              ...
              if (fileItems.length < this.inputFileClass.minFiles) {
                  this._addFieldInvalidClass(singleField);
              } else {
                  this._removeFieldInvalidClass(singleField);
              }
          });
      }
  }
      ...
      if (parentField.length) {
          ...
          if (allEmpty) {
              parentField.forEach((item) => {
                  this._addFieldInvalidClass(item);
              });
          } else {
              parentField.forEach((singleField) => {
  ...
  _addFieldInvalidClass(element) {
      element.classList.add('invalid-field');
  }
  // удаление класса, если поле валидно
  _removeFieldInvalidClass(element) {
